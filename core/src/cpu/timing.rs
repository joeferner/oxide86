...
pub(super) fn calculate_ea_cycles(mode: u8, rm: u8, has_segment_override: bool) -> u64 {
...
pub mod cycles {
    ...
    pub const ADD_REG_REG: u64 = 3;

    /// ADD register to memory: 16 cycles + EA
    pub const ADD_REG_MEM: u64 = 16;

    /// ADD memory to register: 9 cycles + EA
    pub const ADD_MEM_REG: u64 = 9;

    /// ADD immediate to register: 4 cycles
    pub const ADD_IMM_REG: u64 = 4;

    /// ADD immediate to memory: 17 cycles + EA
    pub const ADD_IMM_MEM: u64 = 17;

    /// ADD immediate to accumulator: 4 cycles
    pub const ADD_IMM_ACC: u64 = 4;

    /// ADC (same as ADD)
    pub const ADC_REG_REG: u64 = 3;
    pub const ADC_REG_MEM: u64 = 16;
    pub const ADC_MEM_REG: u64 = 9;
    pub const ADC_IMM_REG: u64 = 4;
    pub const ADC_IMM_MEM: u64 = 17;
    pub const ADC_IMM_ACC: u64 = 4;

    /// SUB (same as ADD)
    pub const SUB_REG_REG: u64 = 3;
    pub const SUB_REG_MEM: u64 = 16;
    pub const SUB_MEM_REG: u64 = 9;
    pub const SUB_IMM_REG: u64 = 4;
    pub const SUB_IMM_MEM: u64 = 17;
    pub const SUB_IMM_ACC: u64 = 4;

    /// SBB (same as ADD)
    pub const SBB_REG_REG: u64 = 3;
    pub const SBB_REG_MEM: u64 = 16;
    pub const SBB_MEM_REG: u64 = 9;
    pub const SBB_IMM_REG: u64 = 4;
    pub const SBB_IMM_MEM: u64 = 17;
    pub const SBB_IMM_ACC: u64 = 4;

    /// INC register: 2 cycles
    pub const INC_REG: u64 = 2;

    /// INC memory: 15 cycles + EA
    pub const INC_MEM: u64 = 15;

    /// DEC register: 2 cycles
    pub const DEC_REG: u64 = 2;

    /// DEC memory: 15 cycles + EA
    pub const DEC_MEM: u64 = 15;

    /// NEG register: 3 cycles
    pub const NEG_REG: u64 = 3;

    /// NEG memory: 16 cycles + EA
    pub const NEG_MEM: u64 = 16;

    /// CMP register with register: 3 cycles
    pub const CMP_REG_REG: u64 = 3;

    /// CMP register with memory: 9 cycles + EA
    pub const CMP_REG_MEM: u64 = 9;

    /// CMP memory with register: 9 cycles + EA
    pub const CMP_MEM_REG: u64 = 9;

    /// CMP immediate with register: 4 cycles
    pub const CMP_IMM_REG: u64 = 4;

    /// CMP immediate with memory: 10 cycles + EA
    pub const CMP_IMM_MEM: u64 = 10;

    /// CMP immediate with accumulator: 4 cycles
    pub const CMP_IMM_ACC: u64 = 4;

    /// MUL register (8-bit): 70-77 cycles (data-dependent, use average)
    pub const MUL_REG8: u64 = 74;

    /// MUL register (16-bit): 118-133 cycles (data-dependent, use average)
    pub const MUL_REG16: u64 = 126;

    /// MUL memory (8-bit): 76-83 cycles + EA
    pub const MUL_MEM8: u64 = 80;

    /// MUL memory (16-bit): 124-139 cycles + EA
    pub const MUL_MEM16: u64 = 132;

    /// IMUL register (8-bit): 80-98 cycles (data-dependent, use average)
    pub const IMUL_REG8: u64 = 89;

    /// IMUL register (16-bit): 128-154 cycles (data-dependent, use average)
    pub const IMUL_REG16: u64 = 141;

    /// IMUL memory (8-bit): 86-104 cycles + EA
    pub const IMUL_MEM8: u64 = 95;

    /// IMUL memory (16-bit): 134-160 cycles + EA
    pub const IMUL_MEM16: u64 = 147;

    /// IMUL with immediate (80186+): 22-25 cycles (register), 25-28 + EA (memory)
    pub const IMUL_IMM_REG: u64 = 24;
    pub const IMUL_IMM_MEM: u64 = 27;

    /// DIV register (8-bit): 80-90 cycles (data-dependent, use pessimistic)
    pub const DIV_REG8: u64 = 90;

    /// DIV register (16-bit): 144-162 cycles (data-dependent, use pessimistic)
    pub const DIV_REG16: u64 = 162;

    /// DIV memory (8-bit): 86-96 cycles + EA
    pub const DIV_MEM8: u64 = 96;

    /// DIV memory (16-bit): 150-168 cycles + EA
    pub const DIV_MEM16: u64 = 168;

    /// IDIV register (8-bit): 101-112 cycles (data-dependent, use pessimistic)
    pub const IDIV_REG8: u64 = 112;

    /// IDIV register (16-bit): 165-184 cycles (data-dependent, use pessimistic)
    pub const IDIV_REG16: u64 = 184;

    /// IDIV memory (8-bit): 107-118 cycles + EA
    pub const IDIV_MEM8: u64 = 118;

    /// IDIV memory (16-bit): 171-190 cycles + EA
    pub const IDIV_MEM16: u64 = 190;
    ...
    pub const AND_REG_REG: u64 = 3;
    pub const AND_REG_MEM: u64 = 16;
    pub const AND_MEM_REG: u64 = 9;
    pub const AND_IMM_REG: u64 = 4;
    pub const AND_IMM_MEM: u64 = 17;
    pub const AND_IMM_ACC: u64 = 4;

    /// OR (same as AND)
    pub const OR_REG_REG: u64 = 3;
    pub const OR_REG_MEM: u64 = 16;
    pub const OR_MEM_REG: u64 = 9;
    pub const OR_IMM_REG: u64 = 4;
    pub const OR_IMM_MEM: u64 = 17;
    pub const OR_IMM_ACC: u64 = 4;

    /// XOR (same as AND)
    pub const XOR_REG_REG: u64 = 3;
    pub const XOR_REG_MEM: u64 = 16;
    pub const XOR_MEM_REG: u64 = 9;
    pub const XOR_IMM_REG: u64 = 4;
    pub const XOR_IMM_MEM: u64 = 17;
    pub const XOR_IMM_ACC: u64 = 4;

    /// NOT register: 3 cycles
    pub const NOT_REG: u64 = 3;
    pub const NOT_MEM: u64 = 16;

    /// TEST register with register: 3 cycles
    pub const TEST_REG_REG: u64 = 3;
    pub const TEST_REG_MEM: u64 = 9;
    pub const TEST_IMM_REG: u64 = 5;
    pub const TEST_IMM_MEM: u64 = 11;
    pub const TEST_IMM_ACC: u64 = 4;
    ...
    pub const SHIFT_REG_1: u64 = 2;

    /// Shift/Rotate memory by 1: 15 cycles + EA
    pub const SHIFT_MEM_1: u64 = 15;

    /// Shift/Rotate register by CL: 8 + 4*count cycles
    pub const SHIFT_REG_CL_BASE: u64 = 8;
    pub const SHIFT_REG_CL_PER_COUNT: u64 = 4;

    /// Shift/Rotate memory by CL: 20 + 4*count cycles + EA
    pub const SHIFT_MEM_CL_BASE: u64 = 20;
    pub const SHIFT_MEM_CL_PER_COUNT: u64 = 4;

    /// Shift/Rotate register by immediate (80186+): 5 + count cycles
    pub const SHIFT_REG_IMM_BASE: u64 = 5;
    pub const SHIFT_REG_IMM_PER_COUNT: u64 = 1;

    /// Shift/Rotate memory by immediate (80186+): 17 + count cycles + EA
    pub const SHIFT_MEM_IMM_BASE: u64 = 17;
    pub const SHIFT_MEM_IMM_PER_COUNT: u64 = 1;
    ...
    pub const JMP_SHORT: u64 = 15;

    /// JMP near direct: 15 cycles
    pub const JMP_NEAR_DIRECT: u64 = 15;

    /// JMP near indirect through register: 11 cycles
    pub const JMP_NEAR_INDIRECT_REG: u64 = 11;

    /// JMP near indirect through memory: 18 cycles + EA
    pub const JMP_NEAR_INDIRECT_MEM: u64 = 18;

    /// JMP far direct: 15 cycles
    pub const JMP_FAR_DIRECT: u64 = 15;

    /// JMP far indirect through memory: 24 cycles + EA
    pub const JMP_FAR_INDIRECT_MEM: u64 = 24;

    /// Conditional jump (taken): 16 cycles
    pub const CONDITIONAL_JUMP_TAKEN: u64 = 16;

    /// Conditional jump (not taken): 4 cycles
    pub const CONDITIONAL_JUMP_NOT_TAKEN: u64 = 4;

    /// LOOP (taken): 17 cycles
    pub const LOOP_TAKEN: u64 = 17;

    /// LOOP (not taken, CX=0): 5 cycles
    pub const LOOP_NOT_TAKEN: u64 = 5;

    /// LOOPE/LOOPZ (taken): 18 cycles
    pub const LOOPE_TAKEN: u64 = 18;

    /// LOOPE/LOOPZ (not taken): 6 cycles
    pub const LOOPE_NOT_TAKEN: u64 = 6;

    /// LOOPNE/LOOPNZ (taken): 19 cycles
    pub const LOOPNE_TAKEN: u64 = 19;

    /// LOOPNE/LOOPNZ (not taken): 5 cycles
    pub const LOOPNE_NOT_TAKEN: u64 = 5;

    /// JCXZ (taken): 18 cycles
    pub const JCXZ_TAKEN: u64 = 18;

    /// JCXZ (not taken): 6 cycles
    pub const JCXZ_NOT_TAKEN: u64 = 6;

    /// CALL near direct: 19 cycles
    pub const CALL_NEAR_DIRECT: u64 = 19;

    /// CALL near indirect through register: 16 cycles
    pub const CALL_NEAR_INDIRECT_REG: u64 = 16;

    /// CALL near indirect through memory: 21 cycles + EA
    pub const CALL_NEAR_INDIRECT_MEM: u64 = 21;

    /// CALL far direct: 28 cycles
    pub const CALL_FAR_DIRECT: u64 = 28;

    /// CALL far indirect through memory: 37 cycles + EA
    pub const CALL_FAR_INDIRECT_MEM: u64 = 37;

    /// RET near: 8 cycles
    pub const RET_NEAR: u64 = 8;

    /// RET near with pop value: 12 cycles
    pub const RET_NEAR_POP: u64 = 12;

    /// RET far: 18 cycles
    pub const RET_FAR: u64 = 18;

    /// RET far with pop value: 17 cycles
    pub const RET_FAR_POP: u64 = 17;

    /// INT (software interrupt): 51 cycles
    pub const INT: u64 = 51;

    /// INT 3 (breakpoint): 52 cycles
    pub const INT3: u64 = 52;

    /// INTO (interrupt on overflow, taken): 53 cycles
    pub const INTO_TAKEN: u64 = 53;

    /// INTO (interrupt on overflow, not taken): 4 cycles
    pub const INTO_NOT_TAKEN: u64 = 4;

    /// IRET (interrupt return): 24 cycles
    pub const IRET: u64 = 24;
    ...
    pub const MOVS: u64 = 18;

    /// REP MOVSB/MOVSW: 9 + 17*CX cycles
    pub const REP_MOVS_BASE: u64 = 9;
    pub const REP_MOVS_PER_ITER: u64 = 17;

    /// CMPSB/CMPSW: 22 cycles per iteration
    pub const CMPS: u64 = 22;

    /// REPE/REPNE CMPSB/CMPSW: 9 + 22*count cycles (count <= CX)
    pub const REP_CMPS_BASE: u64 = 9;
    pub const REP_CMPS_PER_ITER: u64 = 22;

    /// SCASB/SCASW: 15 cycles per iteration
    pub const SCAS: u64 = 15;

    /// REPE/REPNE SCASB/SCASW: 9 + 15*count cycles (count <= CX)
    pub const REP_SCAS_BASE: u64 = 9;
    pub const REP_SCAS_PER_ITER: u64 = 15;

    /// LODSB/LODSW: 12 cycles per iteration
    pub const LODS: u64 = 12;

    /// REP LODSB/LODSW: 9 + 13*CX cycles
    pub const REP_LODS_BASE: u64 = 9;
    pub const REP_LODS_PER_ITER: u64 = 13;

    /// STOSB/STOSW: 11 cycles per iteration
    pub const STOS: u64 = 11;

    /// REP STOSB/STOSW: 9 + 10*CX cycles
    pub const REP_STOS_BASE: u64 = 9;
    pub const REP_STOS_PER_ITER: u64 = 10;
    ...
    pub const IN_IMM: u64 = 10;

    /// IN AL/AX, DX: 8 cycles
    pub const IN_DX: u64 = 8;

    /// OUT imm8, AL/AX: 10 cycles
    pub const OUT_IMM: u64 = 10;

    /// OUT DX, AL/AX: 8 cycles
    pub const OUT_DX: u64 = 8;
    ...
    /// XLAT: 11 cycles
    pub const XLAT: u64 = 11;

    /// LAHF (Load AH from Flags): 4 cycles
    pub const LAHF: u64 = 4;

    /// SAHF (Store AH into Flags): 4 cycles
    pub const SAHF: u64 = 4;

    /// PUSHF: 10 cycles
    pub const PUSHF: u64 = 10;

    /// POPF: 8 cycles
    pub const POPF: u64 = 8;

    /// CLC/STC/CMC/CLD/STD/CLI/STI: 2 cycles
    pub const FLAG_OPS: u64 = 2;
    ...
    /// AAM: 83 cycles
    pub const AAM: u64 = 83;

    /// AAD: 60 cycles
    pub const AAD: u64 = 60;
