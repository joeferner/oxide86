...
// MIGRATED      /// INT 10h, AH=0Eh - Teletype Output
// MIGRATED      /// Input:
// MIGRATED      ///   AL = character to write
// MIGRATED      ///   BL = foreground color (in graphics modes)
// MIGRATED      ///   BH = page number (0 for text mode)
// MIGRATED      /// Output: None
// MIGRATED      pub(crate) fn int10_teletype_output(&mut self, bus: &mut Bus) {
// MIGRATED          let ch = (self.ax & 0xFF) as u8; // AL
// MIGRATED          let cursor = bus.video().get_cursor();
// MIGRATED          let rows = bus.video().get_rows();
// MIGRATED  
// MIGRATED          match ch {
// MIGRATED              b'\r' => {
// MIGRATED                  // Carriage return - move to column 0
// MIGRATED                  bus.video_mut().set_cursor(cursor.row, 0);
// MIGRATED              }
// MIGRATED              b'\n' => {
// MIGRATED                  // Line feed - move to next line
// MIGRATED                  let new_row = if cursor.row >= rows - 1 {
// MIGRATED                      // Need to scroll
// MIGRATED                      self.scroll_up_internal(bus, 1);
// MIGRATED                      rows - 1
// MIGRATED                  } else {
// MIGRATED                      cursor.row + 1
// MIGRATED                  };
// MIGRATED                  bus.video_mut().set_cursor(new_row, cursor.col);
// MIGRATED              }
// MIGRATED              b'\x08' => {
// MIGRATED                  // Backspace
// MIGRATED                  if cursor.col > 0 {
// MIGRATED                      bus.video_mut().set_cursor(cursor.row, cursor.col - 1);
// MIGRATED                  }
// MIGRATED              }
// MIGRATED              _ => {
// MIGRATED                  // Normal character - handle based on video mode
// MIGRATED                  if bus.video().is_graphics_mode() {
// MIGRATED                      // Graphics mode: draw character pixel-by-pixel
// MIGRATED                      let fg_color = (self.bx & 0xFF) as u8; // BL
// MIGRATED                      // AH=0Eh (teletype) draws transparent characters - no background, no XOR
// MIGRATED                      self.draw_char_graphics(
// MIGRATED                          bus,
// MIGRATED                          ch,
// MIGRATED                          cursor.row,
// MIGRATED                          cursor.col,
// MIGRATED                          fg_color,
// MIGRATED                          GraphicsDrawMode::Transparent,
// MIGRATED                      );
// MIGRATED                  } else {
// MIGRATED                      // Text mode: write character byte directly
// MIGRATED                      let cols = bus.video().get_cols();
// MIGRATED                      let offset = (cursor.row * cols + cursor.col) * 2;
// MIGRATED  
// MIGRATED                      // Log existing attribute for debugging
// MIGRATED                      let existing_attr = bus.video().read_byte(offset + 1);
// MIGRATED                      log::debug!(
// MIGRATED                          "INT 10h AH=0Eh: Writing '{}' (0x{:02X}) at ({},{}) ({:04X}h) - existing attr=0x{:02X} (fg={}, bg={})",
// MIGRATED                          if (32..127).contains(&ch) {
// MIGRATED                              ch as char
// MIGRATED                          } else {
// MIGRATED                              '.'
// MIGRATED                          },
...
// MIGRATED                          cursor.col,
// MIGRATED                          offset,
// MIGRATED                          existing_attr,
// MIGRATED                          existing_attr & 0x0F,
// MIGRATED                          (existing_attr >> 4) & 0x07
// MIGRATED                      );
// MIGRATED  
// MIGRATED                      bus.video_mut().write_byte(offset, ch);
// MIGRATED                      // Preserve existing color, but substitute 0x07 for 0x00 (black on black)
// MIGRATED                      // since text with attribute 0x00 is always invisible. Many BIOS implementations
// MIGRATED                      // do this as a compatibility measure for programs that clear the screen with
// MIGRATED                      // attribute 0x00 before exiting (e.g., EDIT, Checkit).
// MIGRATED                      if existing_attr == 0x00 {
// MIGRATED                          bus.video_mut().write_byte(offset + 1, 0x07);
// MIGRATED                      }
// MIGRATED                  }
// MIGRATED  
// MIGRATED                  // Advance cursor
// MIGRATED                  let cols = bus.video().get_cols();
// MIGRATED                  let new_col = cursor.col + 1;
// MIGRATED                  if new_col >= cols {
// MIGRATED                      // Wrap to next line
// MIGRATED                      let new_row = if cursor.row >= rows - 1 {
// MIGRATED                          self.scroll_up_internal(bus, 1);
// MIGRATED                          rows - 1
// MIGRATED                      } else {
// MIGRATED                          cursor.row + 1
// MIGRATED                      };
// MIGRATED                      bus.video_mut().set_cursor(new_row, 0);
// MIGRATED                  } else {
// MIGRATED                      bus.video_mut().set_cursor(cursor.row, new_col);
// MIGRATED                  }
// MIGRATED              }
// MIGRATED          }
// MIGRATED      }
