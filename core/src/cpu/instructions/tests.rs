...
mod arithmetic_tests {
    ...
    use crate::{Bus, memory::Memory, video::Video};

    /// Helper to create a CPU and bus for testing
    fn setup() -> (Cpu, Bus) {
    ...
    fn test_add_8bit_no_carry() {
        ...
        cpu.ax = 0x0003;
        bus.write_u8(0, 0x05); // immediate value
        cpu.add_imm_acc(0x04, &bus);
    ...
    fn test_add_16bit_no_carry() {
        ...
        cpu.ax = 0x0100;
        bus.write_u16(0, 0x1234);
        cpu.add_imm_acc(0x05, &bus);
    ...
    fn test_inc_8bit() {
        ...
        bus.write_u8(0x1000, 0x42);
        bus.write_u8(0, 0x06); // ModR/M: mode=00, op=000 (INC), r/m=110 (direct address)
        bus.write_u16(1, 0x1000); // Direct address

        cpu.inc_dec_rm(0xFE, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x43, "Memory should be 0x43");
        assert!(!cpu.get_flag(cpu_flag::ZERO), "Zero flag should not be set");
...
mod logical_tests {
    ...
    fn test_not_8bit() {
        ...
        bus.write_u8(0x1000, 0x0F);
        bus.write_u8(0, 0x16); // ModR/M: mode=00, op=010 (NOT), r/m=110
        bus.write_u16(1, 0x1000);

        cpu.unary_group3(0xF6, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0xF0, "Memory should be 0xF0");
    ...
    fn test_neg_8bit_positive() {
        ...
        bus.write_u8(0x1000, 0x01);
        bus.write_u8(0, 0x1E); // ModR/M: mode=00, op=011 (NEG), r/m=110
        bus.write_u16(1, 0x1000);

        cpu.unary_group3(0xF6, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0xFF, "Memory should be 0xFF");
        assert!(
            cpu.get_flag(cpu_flag::CARRY),
    ...
    fn test_neg_8bit_zero() {
        ...
        bus.write_u8(0x1000, 0x00);
        bus.write_u8(0, 0x1E); // ModR/M: op=011 (NEG)
        bus.write_u16(1, 0x1000);

        cpu.unary_group3(0xF6, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x00, "Memory should be 0x00");
        assert!(
            !cpu.get_flag(cpu_flag::CARRY),
...
mod shift_rotate_tests {
    ...
    fn test_shl_8bit_by_1() {
        ...
        bus.write_u8(0x1000, 0x42);
        bus.write_u8(0, 0x26); // ModR/M: mode=00, op=100 (SHL), r/m=110
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x84, "Result should be 0x84");
        assert!(
            !cpu.get_flag(cpu_flag::CARRY),
    ...
    fn test_shl_8bit_with_carry() {
        ...
        bus.write_u8(0x1000, 0x80);
        bus.write_u8(0, 0x26); // op=100 (SHL)
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x00, "Result should be 0x00");
        assert!(
            cpu.get_flag(cpu_flag::CARRY),
    ...
    fn test_shr_8bit_by_1() {
        ...
        bus.write_u8(0x1000, 0x42);
        bus.write_u8(0, 0x2E); // ModR/M: op=101 (SHR)
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x21, "Result should be 0x21");
        assert!(
            !cpu.get_flag(cpu_flag::CARRY),
    ...
    fn test_shr_8bit_with_carry() {
        ...
        bus.write_u8(0x1000, 0x43);
        bus.write_u8(0, 0x2E); // op=101 (SHR)
        bus.write_u16(1, 0x1000);
    ...
    fn test_sar_8bit_positive() {
        ...
        bus.write_u8(0x1000, 0x42);
        bus.write_u8(0, 0x3E); // ModR/M: op=111 (SAR)
        bus.write_u16(1, 0x1000);
    ...
    fn test_sar_8bit_negative() {
        ...
        bus.write_u8(0x1000, 0xFE);
        bus.write_u8(0, 0x3E); // op=111 (SAR)
        bus.write_u16(1, 0x1000);
    ...
    fn test_rol_8bit() {
        ...
        bus.write_u8(0x1000, 0x81);
        bus.write_u8(0, 0x06); // ModR/M: op=000 (ROL)
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x03, "Result should be 0x03");
        assert!(
            cpu.get_flag(cpu_flag::CARRY),
    ...
    fn test_ror_8bit() {
        ...
        bus.write_u8(0x1000, 0x81);
        bus.write_u8(0, 0x0E); // ModR/M: op=001 (ROR)
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0xC0, "Result should be 0xC0");
        assert!(
            cpu.get_flag(cpu_flag::CARRY),
    ...
    fn test_rcl_8bit_carry_clear() {
        ...
        cpu.set_flag(cpu_flag::CARRY, false);
        bus.write_u8(0x1000, 0x40);
        bus.write_u8(0, 0x16); // ModR/M: op=010 (RCL)
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x80, "Result should be 0x80");
    ...
    fn test_rcl_8bit_carry_set() {
        ...
        cpu.set_flag(cpu_flag::CARRY, true);
        bus.write_u8(0x1000, 0x40);
        bus.write_u8(0, 0x16); // op=010 (RCL)
        bus.write_u16(1, 0x1000);

        cpu.shift_rotate_group(0xD0, &mut bus);

        assert_eq!(bus.read_u8(0x1000), 0x81, "Result should be 0x81");
        assert!(
            !cpu.get_flag(cpu_flag::CARRY),
...
mod multiply_divide_tests {
    ...
    fn test_mul_8bit() {
        ...
        cpu.ax = 0x0005;
        bus.write_u8(0x1000, 0x06);
        bus.write_u8(0, 0x26); // ModR/M: op=100 (MUL)
        bus.write_u16(1, 0x1000);
    ...
    fn test_mul_8bit_overflow() {
        ...
        cpu.ax = 0x00C8; // 200
        bus.write_u8(0x1000, 0x03);
        bus.write_u8(0, 0x26); // op=100 (MUL)
        bus.write_u16(1, 0x1000);
    ...
    fn test_mul_16bit() {
        ...
        cpu.ax = 1000;
        cpu.dx = 0;
        bus.write_u16(0x1000, 50);
        bus.write_u8(0, 0x26); // op=100 (MUL)
        bus.write_u16(1, 0x1000);

        cpu.unary_group3(0xF7, &mut bus);
    ...
    fn test_imul_8bit_positive() {
        ...
        cpu.ax = 0x0005;
        bus.write_u8(0x1000, 0x06);
        bus.write_u8(0, 0x2E); // ModR/M: op=101 (IMUL)
        bus.write_u16(1, 0x1000);
    ...
    fn test_imul_8bit_negative() {
        ...
        bus.write_u8(0x1000, 0x06);
        bus.write_u8(0, 0x2E); // op=101 (IMUL)
        bus.write_u16(1, 0x1000);
    ...
    fn test_div_8bit() {
        ...
        cpu.ax = 30;
        bus.write_u8(0x1000, 6);
        bus.write_u8(0, 0x36); // ModR/M: op=110 (DIV)
        bus.write_u16(1, 0x1000);
    ...
    fn test_div_8bit_with_remainder() {
        ...
        cpu.ax = 17;
        bus.write_u8(0x1000, 5);
        bus.write_u8(0, 0x36); // op=110 (DIV)
        bus.write_u16(1, 0x1000);
    ...
    fn test_idiv_8bit_positive() {
        ...
        cpu.ax = 17;
        bus.write_u8(0x1000, 5);
        bus.write_u8(0, 0x3E); // ModR/M: op=111 (IDIV)
        bus.write_u16(1, 0x1000);
    ...
    fn test_idiv_8bit_negative_dividend() {
        ...
        bus.write_u8(0x1000, 5);
        bus.write_u8(0, 0x3E); // op=111 (IDIV)
        bus.write_u16(1, 0x1000);
...
mod data_transfer_tests {
    ...
    fn test_mov_imm_to_reg_8bit() {
        ...
        // MOV AL, 0x42
        bus.write_u8(0, 0x42);
        cpu.mov_imm_to_reg(0xB0, &bus); // 0xB0 = MOV AL, imm8
    ...
    fn test_mov_imm_to_reg_16bit() {
        ...
        // MOV AX, 0x1234
        bus.write_u16(0, 0x1234);
        cpu.mov_imm_to_reg(0xB8, &bus); // 0xB8 = MOV AX, imm16
    ...
    fn test_push_pop_reg() {
        ...
        cpu.push_reg16(0x50, &mut bus); // PUSH AX
        assert_eq!(cpu.sp, 0x0FFE, "SP should decrement by 2");
        assert_eq!(bus.read_u16(0x0FFE), 0x1234, "Value should be on stack");

        cpu.pop_reg16(0x5B, &mut bus); // POP BX
    ...
    fn test_pushf_popf() {
        ...
        assert_eq!(bus.read_u16(0x0FFE), 0x0246, "Flags should be on stack");
    ...
    fn test_lea() {
        ...
        cpu.bx = 0x1000;
        cpu.si = 0x0020;
        bus.write_u8(0, 0x40); // ModR/M: mode=01 (8-bit disp), reg=000 (AX), r/m=000 ([BX+SI])
        bus.write_u8(1, 0x10); // 8-bit displacement
...
mod control_flow_tests {
    ...
    fn test_jmp_short_forward() {
        let (mut cpu, mut bus) = setup();
        cpu.ip = 0x0100;
        bus.write_u8(0x0100, 0x10); // Jump forward by 16 bytes
    ...
    fn test_jmp_short_backward() {
        let (mut cpu, mut bus) = setup();
        cpu.ip = 0x0100;
        bus.write_u8(0x0100, 0xF0); // Jump backward by 16 bytes (-16 as i8)
    ...
    fn test_jz_taken() {
        ...
        bus.write_u8(0x0100, 0x10); // Offset

        cpu.jmp_conditional(0x74, &bus); // JZ/JE
    ...
    fn test_jnz_taken() {
        ...
        cpu.jmp_conditional(0x75, &bus); // JNZ/JNE
    ...
    fn test_jc_taken() {
        ...
        cpu.jmp_conditional(0x72, &bus); // JC/JB
    ...
    fn test_jnc_taken() {
        ...
        cpu.jmp_conditional(0x73, &bus); // JNC/JAE
    ...
    fn test_js_taken() {
        ...
        cpu.jmp_conditional(0x78, &bus); // JS
    ...
    fn test_loop_decrement_and_jump() {
        ...
        cpu.ip = 0x0100;
        cpu.cx = 0x0005;
        bus.write_u8(0x0100, 0xF0); // Jump back by 16
    ...
    fn test_call_near_and_ret() {
        ...
        bus.write_u16(0x0100, 0x0050);
        cpu.call_near(&mut bus);
...
mod string_tests {
    ...
    fn test_movsb_forward() {
        ...
        cpu.movs(0xA4, &mut bus); // MOVSB

        assert_eq!(bus.read_u8(0x20000), 0x42, "Byte should be copied");
    ...
    fn test_movsw_backward() {
        ...
        bus.write_u16(0x10010, 0x1234);

        cpu.movs(0xA5, &mut bus); // MOVSW

        assert_eq!(bus.read_u16(0x20010), 0x1234, "Word should be copied");
    ...
    fn test_rep_movsb() {
        ...
        bus.write_u8(0x10000, 0x41);
        bus.write_u8(0x10001, 0x42);
        bus.write_u8(0x10002, 0x43);
        bus.write_u8(0x10003, 0x44);

        cpu.movs(0xA4, &mut bus);

        assert_eq!(bus.read_u8(0x20000), 0x41, "First byte copied");
        assert_eq!(bus.read_u8(0x20001), 0x42, "Second byte copied");
        assert_eq!(bus.read_u8(0x20002), 0x43, "Third byte copied");
        assert_eq!(bus.read_u8(0x20003), 0x44, "Fourth byte copied");
    ...
    fn test_cmpsb_equal() {
        ...
        bus.write_u8(0x10000, 0x42);
        bus.write_u8(0x20000, 0x42);

        cpu.cmps(0xA6, &bus); // CMPSB
    ...
    fn test_lodsw() {
        ...
        bus.write_u16(0x10000, 0x1234);

        cpu.lods(0xAD, &bus); // LODSW
    ...
    fn test_stosb() {
        ...
        cpu.stos(0xAA, &mut bus); // STOSB

        assert_eq!(bus.read_u8(0x20000), 0x42, "Byte should be stored");
        assert_eq!(cpu.di, 0x0001, "DI should increment");
    }
    ...
    fn test_rep_stosw() {
        ...
        cpu.stos(0xAB, &mut bus); // STOSW

        assert_eq!(bus.read_u16(0x20000), 0x1234, "First word stored");
        assert_eq!(bus.read_u16(0x20002), 0x1234, "Second word stored");
        assert_eq!(bus.read_u16(0x20004), 0x1234, "Third word stored");
        assert_eq!(cpu.cx, 0x0000, "CX should be zero");
...
mod flags_8086_tests {
    ...
    fn test_pushf_8086_flag_masking() {
        ...
        let pushed_flags = bus.read_u16(0x0FFE);
    ...
    fn test_cpu_detection_8086() {
        ...
        cpu.pushf(&mut bus);
        let flags_after_clear = bus.read_u16(0x0FFE);
        ...
        bus.write_u16(0x0FFE, 0xF000);
        cpu.sp = 0x0FFE; // Position SP to pop the value we just wrote
        cpu.popf(&mut bus);
        cpu.pushf(&mut bus);
        let flags_after_set = bus.read_u16(0x0FFE);
    ...
    fn test_int_pushf() {
        ...
        bus.write_u16(0x14 * 4, 0x0200); // offset
        bus.write_u16(0x14 * 4 + 2, 0x0000); // segment

        // Execute INT (needs the interrupt number as next byte)
        bus.write_u8(0x0100, 0x14); // INT 0x14
        cpu.int(&mut bus);
    ...
    fn test_iret_popf_restriction() {
        ...
        bus.write_u16(0x0FFA, 0x0100); // IP
        bus.write_u16(0x0FFC, 0x0000); // CS
        bus.write_u16(0x0FFE, 0xFFFF); // FLAGS with all bits set
    ...
    fn test_push_sp_8086_behavior() {
        ...
        let memory = Memory::new();
        let video = Video::new();
        let mut bus = Bus::new(memory, video);
        ...
        let pushed_value = bus.read_u16(addr);
        assert_eq!(
            pushed_value, 0x0FFE,
        ...
        cpu.sp = 0x2000;
        cpu.push_reg16(0x54, &mut bus); // PUSH SP
        let bx_value = cpu.pop(&mut bus); // POP BX
