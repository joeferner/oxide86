; INT 10h Function 06h - Scroll Up in EGA mode 0x0D (320x200 16-colour)
; Tests ega_scroll_up_window.
;
...
;
; Test:
;   Write 0xAA to char row 1, col 0, pixel row 0  (A000:0140)
;     pixel_y = 8, offset = 8 * 40 + 0 = 320 = 0x140
;   Scroll up 1 line across full screen (rows 0-24, cols 0-39)
;   Verify A000:0000 == 0xAA  (moved to char row 0)
;   Verify A000:0140 == 0x00  (char row 1 cleared to black)
...
start:
    ...
    ; char row 0, col 0, pixel row 0 → A000:0000 must be 0xAA
    ; pixel_y = 0  →  offset = 0*40 + 0 = 0
    mov al, [es:0x0000]
    cmp al, 0xAA
    jne fail

    ; char row 1 (now blank) → A000:0140 must be 0x00
    mov al, [es:0x0140]
    cmp al, 0x00
    jne fail
