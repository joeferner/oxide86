...
struct Opl3Slot {
    out: i16,
    fbmod: i16,
    mod_input: ModInput, // replaces *mod
    prout: i16,
    eg_rout: u16,
    eg_out: u16,
    eg_inc: u8,
    eg_gen: u8, // 0=attack, 1=decay, 2=sustain, 3=release
    eg_rate: u8,
    eg_ksl: u8,
    trem_chip: bool, // true → use chip.tremolo, false → 0
    reg_vib: u8,
    reg_type: u8,
    reg_ksr: u8,
    reg_mult: u8,
    ...
    reg_rr: u8,
    reg_wf: u8,
    key: u8, // EGK_NORM | EGK_DRUM bitmask
    pg_reset: bool,
    pg_phase: u32,
    pg_phase_out: u16,
    slot_num: u8,
...
#[derive(Clone, Default)]
struct Opl3Channel {
    slotz: [u8; 2],   // slot indices into chip.slot[]
    pair_idx: u8,     // paired channel index; 0xFF = none
    out: [OutSrc; 4], // output sources (replaces *out[4])
    chtype: u8,       // CH_2OP / CH_4OP / CH_4OP2 / CH_DRUM
    f_num: u16,
    block: u8,
    fb: u8,
    con: u8,
...
pub(crate) struct Opl3Chip {
    ...
    eg_add: u8,
    eg_timer_lo: u8,
    newm: u8, // always 0 for OPL2 compat
    nts: u8,
    rhy: u8,
    vibpos: u8,
    vibshift: u8,
    ...
    writebuf: Vec<Opl3WriteBuf>, // length 1024
}
...
const EG_NUM_ATTACK: u8 = 0;
const EG_NUM_DECAY: u8 = 1;
const EG_NUM_SUSTAIN: u8 = 2;
const EG_NUM_RELEASE: u8 = 3;
...
fn envelope_calc_sin(wf: u8, phase: u16, envelope: u16) -> i16 {
    match wf & 0x07 {
...
fn read_mod_input(chip: &Opl3Chip, mi: ModInput) -> i16 {
    match mi {
        ModInput::Zero => 0,
...
fn slot_calc_fb(slot: &mut Opl3Slot, fb: u8) {
    if fb != 0 {
...
fn slot_write_20(slot: &mut Opl3Slot, data: u8) {
...
fn slot_write_40(chip: &mut Opl3Chip, slot_idx: usize, data: u8) {
...
fn slot_write_60(slot: &mut Opl3Slot, data: u8) {
...
fn slot_write_80(slot: &mut Opl3Slot, data: u8) {
...
fn slot_write_e0(slot: &mut Opl3Slot, data: u8, newm: u8) {
...
fn process_slot(chip: &mut Opl3Chip, slot_idx: usize) {
...
fn generate_4ch(chip: &mut Opl3Chip, buf4: &mut [i16; 4]) {
...
pub(crate) fn generate_resampled(chip: &mut Opl3Chip, buf: &mut [i16; 2]) {
...
pub(crate) fn reset(chip: &mut Opl3Chip, samplerate: u32) {
...
pub(crate) fn write_reg(chip: &mut Opl3Chip, reg: u16, v: u8) {
...
fn write_reg_buffered(chip: &mut Opl3Chip, reg: u16, v: u8) {
