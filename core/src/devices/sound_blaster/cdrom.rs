...
use crate::{disk::cdrom::CdromBackend, utils::bcd_to_dec};
...
pub(super) struct SoundBlasterCdromInner {
    base_port: u16,
    drive_selected: u8,
...
impl SoundBlasterCdromInner {
    pub(super) fn new(base_port: u16, disc: Option<Box<dyn CdromBackend>>, irq_line: u8) -> Self {
        let door_open = false;
    ...
    pub(super) fn io_read_u8(&mut self, port: u16, _cycle_count: u32) -> Option<u8> {
    ...
    pub(super) fn io_write_u8(&mut self, port: u16, val: u8, _cycle_count: u32) -> bool {
        let offset = port.wrapping_sub(self.base_port);
        match offset {
            0 => {
    ...
    pub(super) fn load_disc(&mut self, disc: Box<dyn CdromBackend>) {
        self.disc = Some(disc);
    ...
    pub(super) fn eject_disc(&mut self) {
        self.disc = None;
        self.door_open = true;
        self.state = CdromState::Idle;
        self.result_buf.clear();
