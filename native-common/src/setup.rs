...
pub fn load_disks<V: VideoController>(
    ...
    for (i, path) in hard_disks.iter().enumerate() {
        ...
        // start actually contains a valid FAT BPB before committing to a
        // PartitionedDisk.  Falls back to unpartitioned if not valid.
        let has_partitions = mbr_partition.filter(|p| {
            ...
            if !valid {
                log::warn!(
                    "MBR on {} claims partition at sector {} but no valid FAT BPB found there; treating as unpartitioned",
                    path, p.start_sector
                );
            }
            valid
